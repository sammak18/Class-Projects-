...
class KDTree
{
  ...
  private:
     ...
     */
     Point<Dim> quick_select(vector<Point<Dim>>& Points,int low, int high, int median, int d);
     int partition (vector<Point<Dim>>& Points, int low, int high, int d);
     KDTreeNode* bob_builder(vector<Point<Dim>> copyvector, int d);
    // KDTreeNode* getmax_dada(KDTreeNode* a, KDTreeNode* b, KDTreeNode* target, int Dim);
     KDTreeNode* _copy(const KDTreeNode* subroot);
     void _destroy(KDTreeNode * subroot);
     KDTreeNode* walker( KDTreeNode* current, Point<Dim> target, int dimension, int &best_distance_sqrd ,KDTreeNode*& best_node) const;
};
